...
class UniswapPositionTracker {
  ...
  async checkPositions(): Promise<void> {
    try {
      ...
      for (const position of positions) {
        ...
        console.log(`  Token0: ${position.token0.amount} ${position.token0.symbol} ($${position.token0.valueUSD?.toFixed(2)})`);
        console.log(`  Token1: ${position.token1.amount} ${position.token1.symbol} ($${position.token1.valueUSD?.toFixed(2)})`);
        console.log(`  Total Value: $${position.totalValueUSD?.toFixed(2)}`);
        console.log(`  Fee Tier: ${position.fee / 10000}%`);
        
        if (position.priceRange) {
          const isInRange = position.priceRange.current >= position.priceRange.lower && 
                           position.priceRange.current <= position.priceRange.upper;
          const rangeStatus = isInRange ? '✅ In Range' : '❌ Out of Range';
          console.log(`  Price Range (${position.priceRange.currency}): ${position.priceRange.lower.toFixed(2)} - ${position.priceRange.upper.toFixed(2)}`);
          console.log(`  Current Price: ${position.priceRange.current.toFixed(2)} ${position.priceRange.currency} ${rangeStatus}`);
        } else {
          console.log(`  Price Range: [${position.tickLower}, ${position.tickUpper}]`);
        }
        
        console.log(`  Uncollected Fees:`);
        console.log(`    ${position.token0.symbol}: ${position.uncollectedFees.token0} ($${position.uncollectedFees.token0USD?.toFixed(2)})`);
        console.log(`    ${position.token1.symbol}: ${position.uncollectedFees.token1} ($${position.uncollectedFees.token1USD?.toFixed(2)})`);
  ...
  async start(): Promise<void> {
    ...
    this.scheduler.schedule(config.scheduleTime, () => {
      this.checkPositions();
    });
  ...
  async runOnce(): Promise<void> {
    console.log('🚀 Running Uniswap Position Tracker (One-time check)');
    console.log(`📊 Data will be saved to: ${config.dataFilePath}`);
    console.log('-'.repeat(50));
